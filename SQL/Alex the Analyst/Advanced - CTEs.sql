...
-- Menggunakan Ekspresi Tabel Umum (CTE)
-- CTE memungkinkan Anda menentukan blok subkueri yang dapat direferensikan dalam kueri utama.
-- Ini sangat berguna untuk kueri rekursif atau kueri yang memerlukan referensi tingkat tinggi.
-- Ini adalah sesuatu yang akan kita bahas di pelajaran berikutnya.

-- Mari kita lihat dasar-dasar penulisan CTE:

-- Pertama, CTE dimulai dengan menggunakan Kata Kunci "With". Sekarang kita dapat memberi nama CTE ini sesuka hati.
-- Lalu kita tulis as dan di dalam tanda kurung kita membangun subkueri/tabel yang kita inginkan.
...
WITH CTE_Example(gender, sum_salary, min_salary, max_salary, count_salary,avg_salary) AS
(
SELECT gender,SUM(salary),MIN(salary),MAX(salary),COUNT(salary),AVG(salary)
FROM employee_demographics dem
JOIN employee_salary sal
